...
public interface PedidoRepository extends JpaRepository<Pedido, Integer>{
     ...
     */
	Page<Pedido> findByStatusNotIn(@Param("statusFechado") List<StatusPedido> statusFechado, Pageable pageable);
	
	/**
	 * Busca um pedido carregando apenas os relacionamentos simples (ManyToOne).
	 * As coleções (itens, pagamentos, historico) serão carregadas via Lazy Loading
	 * quando forem acessadas dentro de uma transação ativa.
	 * 
	 * Isso evita o erro "Could not generate fetch" do Hibernate 6+ causado por 
	 * múltiplos JOIN FETCH de coleções (List).
	 */
	@Query("SELECT p FROM Pedido p " +
	       "LEFT JOIN FETCH p.cliente cl " +
	       "LEFT JOIN FETCH cl.endereco " +
	       "LEFT JOIN FETCH p.entregador " +
	       "LEFT JOIN FETCH p.operador " +
	       "WHERE p.id = :id")
	...
	@Query("SELECT p FROM Pedido p " +
	       ...
	       "LEFT JOIN FETCH p.operador " +
	       "WHERE p.id = :id")
	Optional<Pedido> findByIdCompletoParaImpressao(@Param("id") Integer id);
